...
export class LifeV2Repo {
  ...
  async saveTheme(input:{localDate:string;theme:string;toneTags:string[];sourceFactors:string[];themeId?:string}):Promise<LifeDayThemeRow>{const id=input.themeId?`${input.themeId}:${input.localDate}`:newId('theme');await this.db.run(`INSERT INTO life_day_themes(id,local_date,theme,tone_tags_json,source_factors_json,created_at) VALUES(?,?,?,?,?,?) ON CONFLICT(local_date) DO UPDATE SET theme=excluded.theme,tone_tags_json=excluded.tone_tags_json,source_factors_json=excluded.source_factors_json`,[id,input.localDate,input.theme,JSON.stringify(input.toneTags),JSON.stringify(input.sourceFactors),nowIso(this.now)]);return (await this.themeFor(input.localDate))!;}
